				...
				<div class="product-grid-list">
					<div class="row mt-30">
                                  ...
                                  <select name='parent_id'>
                                      @if(isset($parentSubCat) && $parentSubCat->count() > 0)
                                          @foreach ($parentSubCat as $parent)
                                              <option value="{{$parent->id}}">{{$parent->type}}</option>
                                          @endforeach
                                      @endif
							...
							{{-- //	@foreach ($langs as $count=> $lang) --}}
                                  ...
                                  <input class="form-control form-control-lg"name='the_price' type="text" placeholder="The Price" aria-label=".form-control-lg example">
                                  <h3><small type='hidden' id='the_price_er'class='text text-danger'></small></h3>
                                  <textarea id=""name="description"  placeholder="Description *" class="border w-100 p-3 mt-3 mt-lg-4"></textarea>
                                  <h3><small type='hidden' id='description_er'class='text text-danger'></small></h3>
                                  <br>
									<input class="form-control form-control-sm"value="{{isset($lang_maincategory->translation_lang)? $lang_maincategory->translation_lang :''}}" type="hidden"name='translation_lang' placeholder="The Lang" aria-label=".form-control-sm example">
								<h3><small type='hidden' id='translation_lang_er'class='text text-danger' ></small></h3>


							      <div class="form-check form-swi tch">

								<input class="form-check-input" name="statue"value='1' type="checkbox" id="flexSwitchCheckDefault" checked>
